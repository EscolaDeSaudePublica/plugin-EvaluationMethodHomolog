...
$url = $this->controller->createUrl('applyEvaluationsHomolog', [$entity->id]);

$em = $entity->getEvaluationMethod();

?>
<div ng-controller="ApplyHomologEvaluationResults">
<a class="btn btn-primary hltip"  ng-click="editbox.open('apply-consolidated-results-editbox', $event)" title="Aplicar resultado das avaliações de homologação. "> 
    <?php i::_e('Aplicar avaliações'); ?> 
</a>
  
<edit-box id="apply-consolidated-results-editbox" position="bottom" 
            title="<?php i::esc_attr_e('Aplicar resultado das avaliações de homologação') ?>" 
